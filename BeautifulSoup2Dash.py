...
"""
将抓取的BeautifulSoup中文文档转换为Dash文档(http://www.crummy.com/software/BeautifulSoup/bs4/doc.zh/index.html)
"""
...
def create_db(doc_name, index_html_name):
    ...
    for tag in soup.find("div", {"class": "sphinxsidebar"}).findAll("a", href=True):
        name = tag.text.strip()
        name = re.sub("\n+", " ", name)
        name = re.sub(" +", " ", name)
...
copy_file("beautifulsoup", "beautifulsoup.docset")
create_xml("beautifulsoup.docset", "beautifulsoup", "BeautifulSoup", "python",
           "index.html")
create_db("beautifulsoup.docset", "index.html")
